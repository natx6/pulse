 ...
 * backup snapshot taken first). Opening balances flow into suppliers.opening_balance
 * so what the pharmacy already owes a supplier is in Accounts-Payable. */
  ...
  const pickFile = async () => {
    ...
    try {
      const file = await open({
        multiple: false,
        filters: [{ name: "Supplier export", extensions: ["xlsx", "xls", "ods", "csv"] }],
      });
      if (!file || typeof file !== "string") return;
      setFileName(file.split(/[\\/]/).pop() ?? file);
      const s = await parseSupplierFile(file);
    ...
    >
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-outline-variant bg-surface shadow-lg">
        <div className="flex items-center justify-between border-b border-outline-variant px-5 py-3">
          <div>
            <h3 className="text-headline-md font-headline-md text-on-surface">Import Suppliers</h3>
            <p className="text-body-sm font-body-sm text-on-surface-variant">
              Bring over your old system's suppliers and what you owe them.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {phase === "pick" && (
            <div>
              ...
              <p className="mt-4 text-body-sm font-body-sm text-on-surface-variant">
                Everything lands in one transaction. Matching suppliers (by name) get their phone,
                location, and opening balance refreshed; new ones are created. A backup is taken
                automatically before the import. We import the <em>current</em> position — not old
                purchase history.
              </p>
            </div>
          )}
          ...
          {phase === "done" && summary && (
            <div>
              ...
              <p className="text-body-sm font-body-sm text-on-surface-variant">
                Suppliers are live — their opening balances show in Accounts-Payable (Analytics).
              </p>
            </div>
          )}
        </div>
...
const FIELDS: { key: keyof SupplierImportRow; label: string; aliases: string[] }[] = [
  { key: "name", label: "Supplier name", aliases: ["name", "supplier", "vendor", "distributor", "company"] },
  { key: "phone", label: "Phone", aliases: ["phone", "mobile", "tel", "telephone", "contact", "cell"] },
  { key: "location", label: "Location", aliases: ["location", "address", "city", "town", "region"] },
  { key: "opening_balance", label: "Opening balance owed", aliases: ["balance", "owed", "ap", "opening", "opening balance", "amount owed", "outstanding", "credit balance", "balance owed"] },
];
...
/** Convert one raw row to a SupplierImportRow per the current mapping.
 * Returns null when the row has no name (it would be skipped). */
function rowToRecord(row: string[], mapping: Record<string, number>): SupplierImportRow | null {
